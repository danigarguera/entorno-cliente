...
// Cargar los datos del Pokémon del jugador y mostrarlo
window.onload = function() {
    // Obtener el Pokémon del jugador desde la cookie
    const playerPokemon = getCookie('playerPokemon');

    if (playerPokemon) {
        // Mostrar el Pokémon del jugador
        ...
        window.location.href = 'index.html'; // Redirigir al jugador si no se encuentra su Pokémon
    }

    // Seleccionar un Pokémon aleatorio para el oponente
    const enemyPokemon = pokemons[Math.floor(Math.random() * pokemons.length)];

    // Guardar el Pokémon del enemigo en una cookie
    setCookie('enemyPokemon', enemyPokemon, 7);

    // Mostrar el Pokémon del oponente
    ...
    document.getElementById('startBattleButton').addEventListener('click', function() {
        window.location.href = 'battle.html'; // Redirige a la página de batalla
    });
};